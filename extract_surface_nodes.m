...
% |------>x
%
% ! WARNING: Only works for rectangular meshes with the faces
% labelled as they are. If a different labelling system is chosen,
% surf_node_util, must be updated appropriately such that the surface
% normals calculated in the analytic traction function point out of the
% volume. Else the forces will have the wrong sign.
%=====================================================================%
% Inputs
%=====================================================================%
%
...
%       min(x), yz-plane, face 1 ~Sleft
%           surf_node_util(1:6, 1) = [1, 5, 4, 8, yz, 1]
%       max(x), yz-plane, face 2 ~Sright
%           surf_node_util(1:6, 2) = [6, 2, 7, 3, yz, 1]
%       min(y), xz-plane, face 3 ~Sfront
%           surf_node_util(1:6, 3) = [5, 6, 8, 7, xz, 2]
%       max(y), xz-plane, face 4 ~Sback
%           surf_node_util(1:6, 4) = [2, 1, 3, 4, xz, 2]
%       min(z), xy-plane, face 5 ~Sbot
%           surf_node_util(1:6, 5) = [6, 5, 2, 1, xy, 3]
%       max(z), xy-plane, face 6 ~Stop
%           surf_node_util(1:6, 6) = [3, 4, 7, 8, xy, 3]
...
%   S. Queyreau, J. Marian, B.D. Wirth, A. Arsenlis, MSMSE,
%   22(3):035004, (2014).
%
...
% Optional parameters:
%
% surf_node_util := dimension(n_nodes+2, 6). For a rectangular mesh,
%   each column is laid out as follows:
...
[labels, coords] = extract_node_planes(fem_nodes , fem_node_cnct,...
    surf_node_util, fem_planes, n_se, n_nodes);
