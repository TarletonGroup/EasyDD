...
MATLABrelease = ver('MATLAB').Release; % Do this to reduce overhead later
...
straindot = 1e5*timeSI; % Strain rate
...
% Plotting parameters:
plotfreq = Inf; % =1: plot at each time-step, =Inf (or =0): never plot
...
glidecoeffsstruct.dragsessile = 1e2;
...
mobilitystruct.crsssessile = crsssessile;
...
% Frank-Read source: 1Pr
b_frref = slipsystemscartrot{1,2,1};
b_frmag = -norm(b_frref);
b_fr = b_frmag * (b_frref ./ norm(b_frref));
b_frunit = b_fr / norm(b_fr);
n_fr = slipsystemscartrot{1,2,2};

% Vacancy loop: 2Pr
b_vref = slipsystemscartrot{3,2,1};
b_vmag = norm(b_vref);
b_v = b_vmag * (b_vref ./ norm(b_vref));
b_vunit = b_v / norm(b_v);
n_1v = slipsystemscartrot{3,1,2}; % Ba plane
n_2v = slipsystemscartrot{3,2,2}; % Pr plane
...
rnv2_lo = rnvac_lo;
scalecentrnv2_lo = ((dz*1/2 - vaclen*0.8) - centpos_lo(3))/b_frunit(3);
rnv2_lo(:,1:3) = rnvac_lo(:,1:3) + scalecentrnv2_lo * (b_frunit) + [0 0 -vaclen/2];
...
viewaxisframes = [dx*0.98/16 dx*3.2/32 dy*3/8 dy*5/8 dz*0.98/6 dz*8/16];
...
% Plot nodes:
rnfigname = append('rn_',INPUTNAME);
...
savefilefig = append('Figures/',rnfigname,'.fig');
savefig(rnfig,savefilefig,'compact');
close(rnfig);
% To open saved figure: openfig('Figures/rn_xxx.fig')
