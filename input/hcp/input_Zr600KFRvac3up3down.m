...
%addpath([mainpath outputspath]);
%addpath([mainpath outputspath 'Frames']); % Movie frames
...
% Plotting parameters:
plotfreq = 1; % =1: plot at each time-step, =Inf: never plot
...
viewangleframes = [-45,20];
viewaxisframes = [0 dx 0 dy 0 dz];
...
close(rnfig);
% To open saved figure: openfig('Figures/rn_input_Zr600K.fig')
