...
void triangleDisplacement(double *point, double *A, double *B, double *C, double *b, double NU, double *utilda);
                ...
                int nrhs, const mxArray *prhs[])
{
    ...
    triangleDisplacement(point,A,B,C,b,NU,utilda);
}
...
void triangleDisplacement(double *point, double *A, double *B, double *C, double *b, double NU, double *utilda)
{
	...
	safenorm(vecCA, tCA);
/*
    normAB = sqrt(vecAB[0]*vecAB[0] + vecAB[1]*vecAB[1] + vecAB[2]*vecAB[2]);
    normBC = sqrt(vecBC[0]*vecBC[0] + vecBC[1]*vecBC[1] + vecBC[2]*vecBC[2]);
    normCA = sqrt(vecCA[0]*vecCA[0] + vecCA[1]*vecCA[1] + vecCA[2]*vecCA[2]);
    
    eps=1E-10;
    safenorm(vecAB, tAB);
    if (normAB > eps)
       for (i=0; i<3; i++){       
           tAB[i] = vecAB[i]/normAB;
       }
    else
        for (i=0; i<3; i++){          
            tAB[i] = 0;
