function [rn,links,connectivity,linksinconnect,fseg]=separation_c(rn,links,connectivity,linksinconnect,fseg,mobility,MU,NU,a,Ec,mindist,vertices,...
    ...
    if (c>3) && (rn(i,lrn2)==0)
        ...
        for j=1:numsplitmodes
            ...
            if norm(links(size(links,1),6:8))<eps
                ...
                fseg= segforcevec(MU,NU,a,Ec,rn(:,[1 2 3 lrn2]),links,0,vertices,uhat,nc,xnodes,D,mx,mz,w,h,d);
            end
            
            [vntmp1,links,~]=feval(mobility,fseg,rn,links,connectivity,nodelist,clist);
            vntmp=vntmp1(nodelist(:),:);
            vd1=vntmp(1,:)*vntmp(1,:)';
            vd2=vntmp(2,:)*vntmp(2,:)';
            ...
            [vntmp1,links,fntmp]=feval(mobility,fseg,rn,links,connectivity,nodelist,clist);
            vntmp=vntmp1(nodelist(:),:);
...
%             end
            ...
            for k=1:connectivity(i,1)
                ...
                [rn(:,4:6),~]=feval(mobility,fseg,rn,links,connectivity,othernode,clist);
            end
