...
for n=1:L1
    ...
    for i=1:numNbrs
        ...
        if L>0.0
            ...
            linedir=rt./L;
            % DHT
            % The following 3 + 1 lines are commented out to allow non
            % [111]-type dislocations to glide with the glide drag
            % (rather than glide and climb isotropically with the climb
            % drag - a fudge used to reproduce sessile [111] junction
            % formation).
            ...
            %else
                ...
                % DHT
                % The following 4 lines attempt to remove isotropic screw
                % glide and restrict screws to move within their "glide
                % place" only.
                % They replace the 1 line above and the first 2 lines in
                % the subsequent if statement and do not work properly.
                %ndir=links(connectivity(n0,2*ii),6:8); % glide plane normal of the link
                %ndir=ndir./norm(ndir);                 % normalise ndir
                %mdir=cross(ndir,linedir);
                %Btotal=Btotal+(2.0*L).*((Bscrew).*(mdir'*mdir) + (Bline).*(linedir'*linedir));
                %
                if ( 1 - cth2 ) >eps
                    ...
                    % DHT
                    % The following line is adjusted to include the
                    % dislocation specific factors of Beclimb which are
                    % missing up to the point
                    %Bclimb=sqrt( Beclimb^2 + ( Bscrew^2 - Beclimb^2 ) * cth2);
                    Bclimb=sqrt( (Beclimb*sum(burgv*burgv')*(1 - cth2))^2 + ( Bscrew^2 - (Beclimb*sum(burgv*burgv')*(1 - cth2))^2 ) * cth2);
                    %
                    % DHT
                    % The following line attempts to remove isotropic screw
                    % glide and restrict screws to move within their "glide
                    % place" only.
                    % It replaces the above line and does not work
                    % properly.
                    %Bclimb=Beclimb*sum(burgv*burgv')*(1 - cth2);
                    %
                    ...
                    % It replaces the above line and does not work
                    % properly.
                    %Btotal=Btotal+(2.0*L).*((Bglide - Bscrew).*(mdir'*mdir) + (Bclimb).*(ndir'*ndir));
                    %
                end
            %end
            %
