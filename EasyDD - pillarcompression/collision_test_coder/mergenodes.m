function [rn,connectivity,links,linksinconnect,fseg,nodeid]=mergenodes(rn,connectivity,links,linksinconnect,fseg,nodeid,deadnode,~,~,~,~)
...
if(linkid>length(links(:,1)))
    fprintf('try to remove link %d while total number of links is %d',linkid,length(links(:,1)));
    pause
end
...
if(linksinconnectnew(deadlink,:)*linksinconnectnew(deadlink,:)'~=0)
    fprintf('this dead link still has connections and should not be removed');
    pause
end
...
if(connectivitynew(deadnode,1)~=0)
    fprintf('this deadnode still has connections and should not be removed');
    disp(deadnode);
    disp(rnnew);
    disp(connectivitynew);
    pause
end
