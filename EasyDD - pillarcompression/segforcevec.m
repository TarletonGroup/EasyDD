...
if linkid==0 %calculate forces on all segments
    ...
    if do_inclusion ==1
        ...
        [finc0, finc1]=inc_nodalforce_gauss(MU,NU,segments);
...
%     fseg=[fpk, fpk]*0.5+[fs0, fs1]+[fr0, fr1];   %combine all force contributions
    fseg=[fpk, fpk]*0.5+[fs0, fs1]+[fr0, fr1] + [finc0, finc1];   %combine all force contributions
    ...
    if do_inclusion ==1
        %%misfit force due to the inclusions
        [finc0, finc1]=inc_nodalforce_gauss(MU,NU,segments(linkid,:));
%         [finc0, finc1]=inc_nodalforce_accelerate(MU,NU,segments,linkid);
    ...
    fseg=[fpk, fpk]*0.5 + [fs0, fs1] + [fr0, fr1] + + [finc0, finc1]; %sum force contributions
