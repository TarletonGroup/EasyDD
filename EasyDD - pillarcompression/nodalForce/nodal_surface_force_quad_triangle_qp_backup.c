...
/*
 ...
 __float128 *integrals_quad_triangle(__float128 *rv , __float128 *sv , __float128 *yv,
   ...
   // Compiler bugs mean that these auxiliary variables have to be arrays. The code presents unpredictable behaviour otherwise, sfr_sq has to be a minimum size of 8 even though indices 4, 5, 6 and 7 are not used. Only multiples of 4 work up until 24, where this breaks. I suspect this is a memory allocation issuem, hopefully future GCC releases fix this bug.
   __float128 rar1_sq[4], rar1_c[4], sfr_sq[8];
   // integrals
   ...
   __float128 a1m3_s2[8], b1m3_r1[8], b1m3_r2[8];
   __float128 a2m1_s1[8], a2m1_s2[8];
   ...
   */
   ...
   for(int j = 0; j < 4; j++) {
     ...
     // abc0m3
     rar1_sq[j]=rar1[i]*rar1[i];
     rar1_c[j]=rar1_sq[j]*rar1[i];
     sfr_sq[j]=sfr[i]*sfr[i];
     b0m3_r1[i]=1.0q/4.0q*(rdotq[i]*rar1_c[j] +3.0q*(rar1_sq[j]-powq(rdotq[i],2.0q))*b0m1_r1[i]);
     c0m3_r1[i]=1.0q/4.0q*(rrdott[i]*rar1_c[j] +3.0q*(rar1_sq[j]-powq(rrdott[i],2.0q))*c0m1_r1[i]);
     // abc1m3
     b1m3_r1[i]= 1.0q/5.0q*rar1_c[j]*rar1_sq[j]-(d*yv[i]+e*r1)*b0m3_r1[i];
     // abc2m1
     // abc21
     ...
     f313_r1[i]=1.0q/(1.0q-d2)*((2.0q*f111_r1[i]-d*f201_r1[i]+d*yv[i]*b21_r1[i]-sfr_sq[j]*c11_r1[i])+(cd-e)*rv[i]*f213_r1[i]);
     // r2
     aa=(onepf2p2ef);
     i=ir2[j];
     bb=(yv[i]*(cf+d)+g*(f+e));
