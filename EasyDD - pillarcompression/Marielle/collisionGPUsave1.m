...
collGPU=1;
%flag to check if the algorithm has to run. 1 at the beginning, 2 if there is a collision, 0 if there is no collision found.
            
        while collGPU>0  %there was a collision M 
            ...
            while kk<=p && collGPU ~=2
               ...
               collision_condition_is_met=((dist2<mindist2)&(ddist2dt<-eps))|(dist2<eps);
                ...
                if collision_condition_is_met
                    ...
                    collGPU=2; %there was one collision
                    L1=T(kk,1); %M
                    L2=T(kk,2); %M
                    n1s1=inputmin(kk,1); %M
                    n2s1=inputmin(kk,2); %M
                    n1s2=inputmin(kk,3); %M
                    n2s2=inputmin(kk,4); %M
                    s1=inputmin(kk,5); %M
                    s2=inputmin(kk,6); %M
