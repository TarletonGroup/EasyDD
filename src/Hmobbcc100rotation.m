...
%     end
    ...
    for i=1:numNbrs
        ...
        if L>0.0
            ...
            if nmag<eps
                ...
                %                     % pause
                %                 end
                if any(sum(abs(repmat(nplane,12,1)-combinations110),2)/3<0.01) %nplane = <111> type
                ...
                %HY20200708
                if abs(norm(burgv)-sqrt(3)/2)>1E-2
                    Btotal=Btotal+(0.5*L).*((Bclimb).*eye(3)+(Bline-Bclimb).*(linedir'*linedir));
                end
    ...
    %         %         Btotal=2*((0.5*L).*( ( Bglide ).* ( mdir' * mdir ) + ( Bclimb ) .* ( nplane' * nplane ) +( Bline ).*( linedir' * linedir )));
    %         %         Btotal=2*((0.5*L).*( ( Bglide ).* ( mdir' * mdir ) + ( Bclimb ) .* ( nplane' * nplane ) +( Bglide ).*( linedir' * linedir )));
    %     end
    ...
    %         Btotal = Btotal+(0.5*L).*((Bclimb).*eye(3)+(Bline-Bclimb).*(linedirX'*linedirX));
    % %         Btotal=BtotalX+(0.5*L).*((Bclimb).*eye(3)+(Bline-Bclimb).*(linedirX'*linedirX));
    % %         Btotal=(0.5*L).*((Bclimb).*eye(3)+(Bline-Bclimb).*(linedir'*linedir));
    %     end
    %     rcond(Btotal)
    if rcond(Btotal)<eps
        ...
        Btotalp = Btotal + eye(3)*max(max(Btotal))*1E-4;
        Btotalm = Btotal - eye(3)*max(max(Btotal))*1E-4;
        vnp = (Btotalp\fn(n,:)')';
        vnm = (Btotalm\fn(n,:)')';
