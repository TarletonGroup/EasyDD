function [f_dln, f_dln_se, f_dln_node] = analytic_traction(...
    ...
    if CUDA_flag
        ...
        bytesPerUnit = 192; % 6 nodes, 3 entries each, 8 bytes each entry, 6 pointers of 8 bytes each;
        ...
        if ~all(f_dln_se)
            fprintf('analytic_traction.m line 179: parallel tractions not being executed on gpu.')
            pause
        end
