function [vn,fn,links] = mobbcc_bb1b_links(fseg,rn,links,connectivity,nodelist,conlist,concentration)
%mobility law function (model: BCC0)
global Bscrew Bedge Beclimb rotationBCC
...
%             end
            ...
            checkmag=abs(sqrt(3)*0.5-mag);
             if check1==3 && checkmag<= sqrt(eps) || check1==2 && check2==1 && checkmag<= sqrt(eps) || check1==1 && check2==1 && check3==1 && checkmag<= sqrt(eps)
...
%                     Bclimb=sqrt( (Beclimb^2 ) * sinth2 + ( Bscrew^2 ) * costh2);
                    ...
                    if size(ndir,1)>1 || size(ndir_alt,1)>1 || planecheck>eps
                        ...
                        cosratio=1-1*cosdev2;%HY20201113
                        sinratio=1-cosratio;
                        %HY20201113: this is problematic - Bglide can be
                        %complex number
                        Bglide=1 / sqrt( (1/Beclimb^2)*sinratio + ( 1 / Bscrew^2 )*cosratio);
                        Bsclimb=1 / sqrt( (1/Beclimb^2)*cosratio + ( 1 / Bscrew^2 )*sinratio);
                        Btotal=Btotal+mag.*((0.5*L).*(( Bglide ).* ( mdir' * mdir ) + ( Bsclimb  ) .* ( ndir' * ndir )+( Bedge ).*(linedir'*linedir) ));
                    end
...
if isempty(find(imag(vn(n,:))~=0))==0
    disp('complex at vn(n,:) in mobility')
    pause
end
...
if isempty(find(imag(vn)~=0))==0
    disp('complex at vn in mobility')
    pause
end
