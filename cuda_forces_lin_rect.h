                                                   ...
                                                   double *o_sch)
{
  ...
  double // Single integrals.
      a0m1[n_limits],
      b0m1[n_limits], c0m1[n_limits], a1m1[n_limits], b1m1[n_limits], c1m1[n_limits],
 ...
 */
  ...
  */
  ...
  for (int i = 0; i < n_limits; i++)
  {
    ...
    d22m3[i] = cuda_integral_type_5(-d001[i], p_dot_t_q_dot_t * y_sq[i], d11m3[i], i_r[i], b01[i], -i_r[i] * i_s[i], ra[i], i_s[i], a01[i]) + i_y[i] * cuda_integral_type_8(-i_p_dot_t, d10m1[i], -i_q_dot_t, d01m1[i], i_p_dot_t * i_s[i], a1m1[i], i_q_dot_t * i_r[i], b1m1[i]); // checked
    // Triple integrals
    // type_3 = a + b*c + d*e
...
void main_se_cuda_nodal_surface_force_linear_rectangle(int n_se, int n_dln, int threads_per_block, char **argv)
{
  ...
  free(b_arr[0]);
  free(dln_node_arr[0]);
  free(dln_node_arr[1]); //free(x3_arr); free(x4_arr); free(x5_arr); free(x6_arr);
  for (int i = 0; i < n_nodes; i++)
  {
    free(se_node_arr[i]);
  }
