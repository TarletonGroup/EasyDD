...
%     time_lin = toc;
%     
%     fxtot_array = fx3_array + fx4_array + fx5_array + fx6_array;
%     %test = fxtot_array - fxtot_array2
%     
%     tic;
...
%     plot(rel_err, '.')
%     xlabel('Surface Node', 'Interpreter', 'latex');
...
%     xlim([0, size(rel_err,1)])
%     ylim([min_rel_err,max_rel_err])
%     saveas(fig1, sprintf('error%d',mx), 'epsc');
%     
%     h=figure;
%     subplot(3,2,1);
%     
...
%  title('Mean RMS $\mathbf{f}^{\mathrm{N}}$ vs. $\mathbf{f}^{\mathrm{A}}$', 'Interpreter', 'latex', 'FontSize', 14)
