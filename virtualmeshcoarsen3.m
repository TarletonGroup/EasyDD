...
lcrit=10^3;   %this is an arbitrary distance and should be corrected to be related to the loop extension distance
node_id=1;   
while node_id<=size(rnnew,1)    %start from the top of rn and go through all nodes
    if connectivitynew(node_id,1)==2 && rnnew(node_id,end)==67   %target only virtual nodes with two connections
            link_node1=linksnew(connectivitynew(node_id,2),3-connectivitynew(node_id,3));   %first noded linked to target node
            ...
            if rnnew(link_node1,end)==67 && rnnew(link_node2,end)==67   %only if both nodes linked to target node are virtual
                ...
                if norm(link_vec1)<lcrit   %if length of link 1 is below critical size then merge
                ...
                elseif norm(link_vec2)<10^3  %if length of link 2 is below critical size then merge
