...
function [rnnew,linksnew,connectivitynew,linksinconnectnew,fsegnew]=virtualmeshcoarsen3(rnnew,linksnew,connectivitynew,linksinconnectnew,fsegnew,MU,NU,a,Ec,dx,dy,dz,lmin)
...
% acrit=0.5*(lcrit^2)*sin(pi/180);   %this is an area related to a desired resolution of angle change and can be altered as required using the sine term
thetacrit=350*pi/180;
node_id=1;
...
while node_id<=size(rnnew,1)    %start from the top of rn and go through all nodes
    ...
    if connectivitynew(node_id,1)==2 && rnnew(node_id,end)==67   %target only virtual nodes with two connections
            ...
            if rnnew(link_node1,end)==67 && rnnew(link_node2,end)==67   %only if both nodes linked to target node are virtual
                ...
                angle=acos((link_vec1/norm(link_vec1))*(link_vec2/norm(link_vec2))');   %angle between link 1 and link 2
%                 area=0.5*norm(link_vec1)*norm(link_vec2)*sin(angle);   %area of triangle formed by link 1 and link 2
                
                if norm(link_vec1)<lcrit && angle<thetacrit || norm(link_vec1)<lmin  %if length of link 1 and the angle change are below critical size then merge
                ...
                elseif norm(link_vec2)<lcrit && angle<thetacrit || norm(link_vec2)<lmin  %if length of link 2 and the angle change are below critical size then merge
