...
mx=15; % number of elements along beam length
loading=1; 
vertices = [0,0,0;...
            dx,0,0;...
            0,dy,0;...
...
[rn,links] = checkGenerator101(NUM_SOURCES,DIST_SOURCE,CRYSTAL_STRUCTURE,dx,dy,dz);
 rn2=rn;
 rn2(:,1)=rn(:,1)+1.5*DIST_SOURCE; %translating of the second loop 
 rn2(:,3)=rn(:,3)+1.5*DIST_SOURCE;
 rn=[rn ; rn2]; %rn concatenation
 load('2loopslinks'); %links loading
 links(:,3:5)=-links(:,3:5);
