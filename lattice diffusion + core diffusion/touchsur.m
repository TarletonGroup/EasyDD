...
% bring in array 'links_cross'. added by gaoyuan 09-11-20
    ...
    while i<=size(links,1)
        ...
        elseif ( abs(rn(n1,lable))<=surpos ) & ( abs(rn(n2,lable))<=surpos ) 
            if(connectivity(n1,1)==1) & (connectivity(n2,1)==1)
                if (rn(n1,lrn2)<3) & (rn(n2,lrn2)<3)
                    mergenode1=n1;
                    mergenode2=n2;
                    [rn,connectivity,links,linksinconnect,fseg,mergednodeid]=mergenodes(rn,connectivity,links,linksinconnect,fseg,mergenode1,mergenode2,MU,NU,a,Ec);    
                end  
            elseif rn(n1,lrn2)<0 & (connectivity(n1,1)==1) & (connectivity(n2,1)==2)
                if connectivity(n2,2)~=connectivity(n1,2)
                   n3=links(connectivity(n2,2),3-connectivity(n2,3));
                else
                   n3=links(connectivity(n2,4),3-connectivity(n2,5));
                end
                    if rn(n3,lrn2)<0 | rn(n2,lrn2)<0
                        mergenode1=n1;
                        mergenode2=n2;
                        [rn,connectivity,links,linksinconnect,fseg,mergednodeid]=mergenodes(rn,connectivity,links,linksinconnect,fseg,mergenode1,mergenode2,MU,NU,a,Ec);    
                    end
            elseif rn(n2,lrn2)<0 & (connectivity(n2,1)==1) & (connectivity(n1,1)==2)
                if connectivity(n1,2)~=connectivity(n2,2)
                   n3=links(connectivity(n1,2),3-connectivity(n1,3));
                else
                   n3=links(connectivity(n1,4),3-connectivity(n1,5));
                end
                if rn(n3,lrn2)<0 | rn(n1,lrn2)<0
                    mergenode1=n1;
                    mergenode2=n2;
                    [rn,connectivity,links,linksinconnect,fseg,mergednodeid]=mergenodes(rn,connectivity,links,linksinconnect,fseg,mergenode1,mergenode2,MU,NU,a,Ec);    
                end
            elseif rn(n1,lrn2)<0 & rn(n2,lrn2)<0
                    mergenode1=n1;
                    mergenode2=n2;
                    [rn,connectivity,links,linksinconnect,fseg,mergednodeid]=mergenodes(rn,connectivity,links,linksinconnect,fseg,mergenode1,mergenode2,MU,NU,a,Ec);    
            end    
...
%     links_crossnew(deadlink,:)=links_crossnew(llinks,:);    %added by gaoyuan 09-11-20
end
linksnew(llinks,:)=[];
% links_crossnew(llinks,:)=[];    %added by gaoyuan 09-11-20
fsegnew(llinks,:)=[];
