function [rn,links,connectivity,linksinconnect,fseg,mmmm]=collision(mmmm,rn,links,connectivity,linksinconnect,fseg,mindist,MU,NU,a,Ec,mobility,appliedstress,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv)
...
while i<size(links,1)
    ...
    while j<=size(links,1)
        ...
        n1s2=links(j,1);
        n2s2=links(j,2);
        if (n1s1~=n1s2)&(n1s1~=n2s2)&(n2s1~=n1s2)&(n2s1~=n2s2) %segment don't share node
            ...
            if collision_condition_is_met
                ...
                elseif close_to_n2s1
                    ...
                    %get the nodeid and link (the last oen in rn) that will be merged in Merge operation. 
                    mergenode1=length(rn(:,1)); 
                    linknew=length(links(:,1));
                    links(linknew,6:8)=links(i,6:8); %slip plane
                    fseg=[fseg;zeros(1,6)];
                end
...
%                 pause
                if mergednodeid>0
                    ...
                    [rn(mergednodeid,4:6),vn_c,~]=feval(mobility,fseg,rn,links,connectivity,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,mergednodeid,conlist);
                end
            end
        end
        j=j+1;
