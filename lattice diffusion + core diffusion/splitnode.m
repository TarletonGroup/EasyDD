...
if testl==0 || testl==connectivity(nodeid,1) || max(splitconnections)>connectivity(nodeid,1)
    %sanity check%
...
if burgv*burgv'~=0
    ...
    links(lastlink,1:8)=[ 0 0 burgv 0 0 0];
    links(lastlink,posnew)=nodeid;
    links(lastlink,3-posnew)=newnodeid;
    %   links(lastlink,8)=links(connectivity(nodeid,2),8);
