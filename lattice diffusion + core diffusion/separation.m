function [rn,links,connectivity,linksinconnect,fseg,mmmm]=separation(mmmm,rn,links,connectivity,linksinconnect,fseg,mobility,MU,NU,a,Ec,mindist,appliedstress,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv)
...
for i=1:lrn
    ...
    if c>3 && rn(i,lrn2)==0
        ...
        for j=1:numsplitmodes
            ...
            if norm(links(size(links,1),6:8))<eps
                fseg=[fseg;0 0 0 0 0 0];
            else
                fseg= segforcevec(MU,NU,a,Ec,rn(:,[1 2 3 lrn2]),links,appliedstress,0);
            end
            [vntmp,~,~]=feval(mobility,fseg,rn,links,connectivity,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,nodelist,clist);
 ...
 %               fseg = segforcevec(MU,NU,a,Ec,rn(:,[1 2 3 lrn2]),links,appliedstress,0);
            ...
            [vntmp,~,fntmp]=feval(mobility,fseg,rn,links,connectivity,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,nodelist,clist);
            ...
            connectivity(i,1:9)=refconnecti(1:9);
            for k=1:c
        ...
        if splittingmode~=0
            ...
            for k=1:connectivity(i,1)
                ...
                [rn(othernode,4:6),vn_c,~]=feval(mobility,fseg,rn,links,connectivity,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,othernode,clist);
            end
