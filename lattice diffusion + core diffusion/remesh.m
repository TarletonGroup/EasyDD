% modified by Ally 2015-3-31
% modified by gaoyuan 09-10-03
% adding the array 'cn' into the program

function [rn,links,connectivity,linksinconnect,fseg]=remesh(rn,links,connectivity,linksinconnect,fseg,lmin,lmax,areamin,areamax,MU,NU,a,Ec,appliedstress,mobility,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv)

% first coarsen the parts of the mesh that can be coarsened
...
[rn,links,connectivity,linksinconnect,fseg]=meshcoarsen(rn,links,connectivity,linksinconnect,fseg,lmin,lmax,areamin,MU,NU,a,Ec,appliedstress,mobility,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv);
[rn,links,connectivity,linksinconnect,fseg]=meshrefine(rn,links,connectivity,linksinconnect,fseg,lmin,lmax,areamax,MU,NU,a,Ec,appliedstress,mobility,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv);

function [rnnew,linksnew,connectivitynew,linksinconnectnew,fsegnew]=meshcoarsen(rn,links,connectivity,linksinconnect,fseg,lmin,lmax,areamin,MU,NU,a,Ec,appliedstress,mobility,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv)
rnnew=rn;
[lrn lrn2]=size(rn);
linksnew=links;
                    ...
                    for j=1:connectivitynew(mergednodeid,1)
                        ...
                        if ((connode==i)|(connode==link1_nodenoti))
                            ...
                            for j=1:2
                                ...
                                [rnnew(nodelist,4:6),vn_c,~]=feval(mobility,fsegnew,rnnew,linksnew,connectivitynew,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,nodelist,clist);
                            end    
                        end
                    end
                end    
...
function [rnnew,linksnew,connectivitynew,linksinconnectnew,fsegnew]=meshrefine(rn,links,connectivity,linksinconnect,fseg,lmin,lmax,areamax,MU,NU,a,Ec,appliedstress,mobility,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv)
[lrn lrn2] = size(rn);
lrn3 = lrn2-1;
rnnew = rn;
linksnew = links;
    ...
    if (connectivitynew(i,1)==2) & (rnnew(i,lrn2)==0)
        ...
        if (((area2>areamax2)&(r2>=lmin2)&(link2_nodenoti<=lrn))|(r2>lmax))
            ...
            for j=1:connectivitynew(newnode,1)
                ...
                fsegnew=segforcevec(MU,NU,a,Ec,rnnew(:,[1 2 3 lrn2]),linksnew,appliedstress,0);
                clist=[connectivitynew(oldnode,1) linspace(1,connectivitynew(oldnode,1),connectivitynew(oldnode,1))];
                [rnnew(oldnode,4:6),vn_c,~]=feval(mobility,fsegnew,rnnew,linksnew,connectivitynew,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,oldnode,clist);
            end
            clist=[connectivitynew(newnode,1) linspace(1,connectivitynew(newnode,1),connectivitynew(newnode,1))];
            [rnnew(newnode,4:6),vn_c,~]=feval(mobility,fsegnew,rnnew,linksnew,connectivitynew,xnodes,mno,wx,wy,wz,mx,my,mz,mel,nc,ke,Dv,newnode,clist);
        end
