function rn=consistencycheck2(rn,vertices)

% Flag all nodes outside of medium.
in = inhull(rn(:,1:3), vertices); %node with flag 1 is inside domain, 0 is outside domain.
rn_size = size(rn,1);

for i=1:rn_size
    %if node is already flagged as virtual fixed node, or is internal
    %skip to next node
    if rn(i,end) == 67 || in(i) == 1
        continue;
    end
    
    if rn(i,end)==0 && in(i)==0
        rn(i,end) = 67;
        disp(strcat('Node ',num2str(i),' outside of domain. Changed to virtual node.'));
    end
end
end
